...
if (!html.includes('js/capcut-ui.js')) {
  throw new Error('Le contrôleur de la nouvelle interface n’est pas chargé.');
}
...
for (const file of scripts) {
  ...
  if (result.status !== 0) {
    throw new Error(`Erreur JavaScript dans ${file}\n${result.stderr}`);
  }
}

console.log(`Validation réussie : ${requiredIds.length} éléments et ${scripts.length} scripts vérifiés.`);
